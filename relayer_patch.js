...
const DESTINATION_TARGET = process.env.DESTINATION_TARGET || "mainnet";
const OPTIMISM_CONFIGS = {
    ...
    mainnet:          { chainId:1,        name:"Ethereum Mainnet", rpc:"https://virginia.rpc.blxrbdn.co", explorer:"https://etherscan.io" },
};
...
async function fetchBN() {
    ...
    const d = await r.json(); return parseFloat(d?.result?.XETHZUSD?.c?.[0] || 0);
}
...
// ── Push price to wPETH on any chain ─────────────────────
async function pushPrice(wpethContract, price, chainName) {
    try {
        if (!price.avg || price.avg === 0) return;
        const avg = BigInt(Math.round(price.avg * 1e8));
        const cg  = BigInt(Math.round((price.cg_usd  || price.avg) * 1e8));
        const kn  = BigInt(Math.round((price.bn_usd  || price.avg) * 1e8));
        const fd  = await wpethContract.runner.provider.getFeeData();
        const tx  = await wpethContract.updateEthPrice(avg, cg, kn, price.source,
            { gasLimit: 200_000n, gasPrice: fd.gasPrice || 5_000_000_000n });
        console.log(`   💰 [${chainName}] wPETH price → $${price.avg} TX: ${tx.hash.slice(0,14)}…`);
    } catch(e) {
        console.warn(`   ⚠  [${chainName}] price push failed: ${e.message.slice(0,60)}`);
    }
}
    ...
    for (const rpc of [BSC_RPC_URL, BSC_RPC_FALLBACK]) {
        try {
            ...
            if (Number(net.chainId) === BSC_CHAIN_ID) {
                console.log(`✅ BSC RPC connected: ${rpc}`);
                return p;
            }
        } catch {}
    }
    console.warn("⚠  BSC RPC unavailable — BSC minting disabled");
    return null;
}
...
`);
    ...
    // ══════════════════════════════════════════════════════
    // LEG C — HTLC MONITORING (private chain events)
    // ══════════════════════════════════════════════════════
    if (htlcPrivate) {
        htlcPrivate.on("SwapCreated", async (swapId, initiator, participant, amount, hashlock, timelock) => {
            const amtETH = ethers.formatEther(amount);
            const exp    = new Date(Number(timelock) * 1000).toLocaleString();
            console.log(`\n🔒 [LEG C] HTLC SwapCreated`);
            ...
            if (dbOk) {
                db.query(
                    `INSERT INTO htlc_swaps (swap_id,chain_id,event_type,initiator,participant,amount_eth,hashlock,timelock,status,created_at)
                     VALUES ($1,$2,'SwapCreated',$3,$4,$5,$6,$7,'waiting_buyer',NOW())
                     ON CONFLICT (swap_id) DO NOTHING`,
                    [swapId, 123456, initiator, participant||null, parseFloat(amtETH), hashlock, Number(timelock)]
                ).catch(() => {});
            }
        });

        htlcPrivate.on("SwapClaimed", async (swapId, secret) => {
            console.log(`\n✅ [LEG C] HTLC SwapClaimed: ${swapId.slice(0,20)}…`);
            console.log(`   Secret revealed — buyer can now claim private ETH`);
            if (dbOk) {
                db.query(
                    `UPDATE htlc_swaps SET status='claimed', secret_revealed=$2, updated_at=NOW() WHERE swap_id=$1`,
                    [swapId, secret]
                ).catch(() => {});
            }
        });

        htlcPrivate.on("SwapRefunded", (swapId) => {
            console.log(`\n↩  [LEG C] HTLC SwapRefunded: ${swapId.slice(0,20)}… (expired)`);
            if (dbOk) {
                db.query(
                    `UPDATE htlc_swaps SET status='refunded', updated_at=NOW() WHERE swap_id=$1`,
                    [swapId]
                ).catch(() => {});
            }
        });

        console.log(`✅ [LEG C] HTLC listener active: ${HTLC_PRIVATE_ADDR}`);
    }
...
console.log(`✅ [LEG A] Listening: ETH bridge on ${BRIDGE_ADDRESS}`);
    ...
    // ── Manual CLI commands ────────────────────────────────
    // node relayer2.js mint-bsc <requestId> [amount]
    // node relayer2.js price-bsc
    const [,,cliCmd, cliArg1, cliArg2] = process.argv;
    if (cliCmd === "mint-bsc" && cliArg1) {
        if (!wpethBSC) { console.error("❌ WPETH_BSC_ADDRESS not set or BSC RPC unavailable"); process.exit(1); }
        const amt = cliArg2 ? ethers.parseEther(cliArg2) : ethers.parseEther("1");
        const bscReqId = ethers.keccak256(ethers.solidityPacked(["bytes32","uint256"], [cliArg1, BigInt(BSC_CHAIN_ID)]));
        const fd  = await bscProvider.getFeeData();
        const tx  = await wpethBSC.mint(bscWallet.address, amt, bscReqId, { gasLimit:200_000n, gasPrice:fd.gasPrice||5_000_000_000n });
        console.log(`✅ BSC mint TX: ${tx.hash}`); await tx.wait(); process.exit(0);
    }
    if (cliCmd === "price-bsc") {
        const price = await fetchDualPrice();
        if (wpethBSC) await pushPrice(wpethBSC, price, "BSC");
        if (wpeth)    await pushPrice(wpeth,    price, "Mainnet");
        process.exit(0);
    }
    ...
    setInterval(async () => {
        try {
            ...
            console.log(`♻  Private #${pb.value?.toLocaleString()||"—"} | Mainnet #${mb.value?.toLocaleString()||"—"} | ETH $${price.avg} [${price.source}] CG:$${price.cg_usd||"—"} BN:$${price.bn_usd||"—"}`);
            // Push live ETH price to wPETH contract every 60s
            if (wpeth && price.avg > 0) {
await pushPrice(wpeth, price, "Mainnet").catch(() => {});
} if (wpethBSC && price.avg > 0) {
                try {
