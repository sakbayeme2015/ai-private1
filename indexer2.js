//  indexer.js — OpenClaw Chain Indexer
//  Private → Mainnet Direct Bridge (no L2)
//  Dual CoinGecko + Binance oracle on BOTH chains per tx
//  Nginx SSL config tracking · P2P peers · Auto DB migration
// ============================================================
...
const ETH_MAINNET_RPC_URL  = process.env.ETH_MAINNET_RPC_URL || "https://eth-mainnet.public.blastapi.io";
...
async function fetchBN() {
    ...
    const d=await res.json(); return parseFloat(d?.result?.XETHZUSD?.c?.[0]||0);
}
...
async function fetchUSDTPrice() {
    ...
    const [privSupply, mainSupply] = await Promise.allSettled([
        ...
        USDT_MAINNET_ADDRESS ? new ethers.Contract(USDT_MAINNET_ADDRESS,["function totalSupply() view returns (uint256)"],new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io")).totalSupply().then(s=>ethers.formatUnits(s,6)).catch(()=>"—") : Promise.resolve("—"),
    ]);
...
async function indexWPETHEvents() {
    ...
    const wpeth = new ethers.Contract(WPETH_ADDRESS, WPETH_EVENTS_ABI, new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io"));
  ...
  if(false) wpeth.on("Minted", async (to, amount, requestId, ethPrice, ev) => {
        const amtETH = parseFloat(ethers.formatEther(amount));
...
`);
  ...
  if(false) wpeth.on("PriceUpdated", async (avgUSD, cgUSD, bnUSD, priceSrc, timestamp) => {
