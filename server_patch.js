//  server.js — OpenClaw Chain Explorer
//  Private Chain → Ethereum Mainnet DIRECT bridge (no L2)
//  Dual CoinGecko + Binance price oracle on BOTH chains
//  Bridge Price Oracle per contract address
//  Nginx SSL proxy (ai-private.online) config + P2P routes
//  Auto DB migration · /tx/ /block/ /address/ detail pages
// ============================================================
...
initBSCProvider().then(() => {
 ...
 address:WPETH_ADDRESS, total_supply:"0", eth_price_usd:0, cg_usd:0, bn_usd:0, source:"—", updated_at:null };
...
async function fetchWPETHStats() {
    ...
    try {
        ...
        const _p = new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io");
...
async function fetchUSDTPrice() {
    ...
    if (mainnetProvider && USDT_MAINNET_ADDRESS) {
        try {
            const _pu = new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io");
            const usdt = new ethers.Contract(USDT_MAINNET_ADDRESS, ERC20_ABI, _pu);
...
app.get("/api/v1/usdt-price", async(req,res) => {
    ...
    const [privBal, mainBal] = await Promise.allSettled([
        ...
        USDT_MAINNET_ADDRESS ? new ethers.Contract(USDT_MAINNET_ADDRESS,ERC20_ABI,new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io")).totalSupply().then(s=>ethers.formatUnits(s,6)) : Promise.resolve("—"),
    ]);
    res.json({
...
app.get("/api/v1/wpeth/balance/:address", async (req, res) => {
    ...
    try {
        const _p2 = new ethers.JsonRpcProvider("https://eth-mainnet.public.blastapi.io");
