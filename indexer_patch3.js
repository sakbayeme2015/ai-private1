...
async function fetchUSDTPrice() {
    ...
    const [privSupply, mainSupply] = await Promise.allSettled([
        ...
        USDT_MAINNET_ADDRESS ? new ethers.Contract(USDT_MAINNET_ADDRESS,["function totalSupply() view returns (uint256)"],new ethers.JsonRpcProvider("https://virginia.rpc.blxrbdn.com")).totalSupply().then(s=>ethers.formatUnits(s,6)).catch(()=>"—") : Promise.resolve("—"),
    ]);
...
async function indexWPETHEvents() {
    ...
    const wpeth = new ethers.Contract(WPETH_ADDRESS, WPETH_EVENTS_ABI, new ethers.JsonRpcProvider("https://virginia.rpc.blxrbdn.com"));
    ...
    try {
        // // const minted = await wpeth.queryFilter(wpeth.filters.Minted(), 0x186614C, "latest"); // disabled - RPC range limit // disabled - RPC range limit
                const minted = []; // disabled queryFilter
    ...
    try {
        // // const prices = await wpeth.queryFilter(wpeth.filters.PriceUpdated(), 0x186614C, "latest"); // disabled - RPC range limit // disabled - RPC range limit
                const prices = []; // disabled queryFilter
